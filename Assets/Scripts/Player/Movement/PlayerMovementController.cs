...
public class PlayerMovementController : Controller {
	...
	protected PlayerPhysicsController phyCtrl;
	...
	void Awake () {
		phyCtrl = GetComponent<PlayerPhysicsController>();
		tr = transform;
	...
	void ControllerUpdate()
	{
		...
		phyCtrl.SetExtendSensorRange(IsGrounded());

		//Set mover velocity;		
		phyCtrl.SetVelocity(_velocity);
	...
	ControllerState DetermineControllerState()
	{
		...
		bool _isSliding = phyCtrl.IsGrounded() && IsGroundTooSteep();
		...
		if(currentControllerState == ControllerState.Falling)
		{
			...
			if(phyCtrl.IsGrounded() && !_isSliding){
    ...
    void HandleMomentum()
	{
		...
		if(currentControllerState == ControllerState.Sliding)
		{
			...
			Vector3 _pointDownVector = Vector3.ProjectOnPlane(phyCtrl.GetGroundNormal(), tr.up).normalized;
		...
		if(currentControllerState == ControllerState.Sliding)
		{
			...
			momentum = Vector3.ProjectOnPlane(momentum, phyCtrl.GetGroundNormal());
			...
			Vector3 _slideDirection = Vector3.ProjectOnPlane(-tr.up, phyCtrl.GetGroundNormal()).normalized;
	...
	private bool IsGroundTooSteep()
	{
		...
		return (Vector3.Angle(phyCtrl.GetGroundNormal(), tr.up) > slopeLimit);
	}
