...
public class Platforms : MonoBehaviour {
    ...
    public bool IsNeighbour(GameObject obj, bool checkPawn)
    {
        bool toReturn = false;
        ...
        if ((checkPawn && !Physics.Raycast(obj.transform.position,Vector3.up,out hit,Mathf.Infinity)) || !checkPawn)
        {
            for (int i = 0; i < 4; i++)
            {
                Vector3 rayDirection = i == 0 ? Vector3.forward : i == 1 ? Vector3.right : i == 2 ? Vector3.back : Vector3.left;
    ...
    public void UpdateLightsGuides()
    {
        GetComponent<TerminalPlatformLight>().ResetLight();
        CameraRigg cRigg = GameObject.FindObjectOfType<CameraRigg>();
            ...
            for (int i = 0; i < otherNeighbours.Count; i++)
            {
                if (cRigg.CurrentPos + 1 == otherNeighbours[i].cameraView)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        Vector3 check = origin + (dir * inc);
                        if (check == otherNeighbours[i].platformNeighbour.transform.position)
                            GetComponent<TerminalPlatformLight>().UpdateLight(true, (j == 0 ? BOX_LIGHT.FORWARD : j == 1 ? BOX_LIGHT.RIGHT : j == 2 ? BOX_LIGHT.BACK : BOX_LIGHT.LEFT));
                    }
                }
            }
        }
